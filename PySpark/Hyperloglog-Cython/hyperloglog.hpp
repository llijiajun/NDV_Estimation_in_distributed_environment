...
public:
    ...
    /**
     * Merges the estimate Array into this object, returning the estimate of array union.
     * The number of registers in each must be the same. Avoid defining a object of HLL in Cython.
     *
     * @param[in] other HyperLogLog's Array to be merged
     * 
     * @exception std::invalid_argument number of registers doesn't match.
     */
...
// using C++ to scan table to speed up.
class Data2Sketch{
public:
    HyperLogLog NDV;
    vector<HyperLogLog> fi;
    uint8_t b_;
    uint8_t order;
    Data2Sketch(uint8_t b=4,uint8_t level=1) throw (std::invalid_argument) :
            b_(b) , order(level){
          if (b_ < 4 || 30 < b_) {
            throw std::invalid_argument("bit width must be in the range [4,30]");
         }
        ...
        for(size_t i=0;i<order;i++){
            HyperLogLog tmp=HyperLogLog(b_);
            fi.push_back(tmp);
        }
    }
    void ScanString(vector<string> &ss){
        sort(ss.begin(),ss.end());
        int count=1;
        int n_string=ss.size();
        int curr=0;
        for(int i=1;i<n_string;i++){
            if(! ss[curr].compare(ss[i])){
                count+=1;
            }else{
                NDV.add(ss[curr].c_str(), ss[curr].size());
                if(count<=order)
                    fi[count-1].add(ss[curr].c_str(), ss[curr].size());
                count=1;
                curr=i;
            }
        }
    }
    void ScanString2(vector<string> &ss){
    ...
    void print(){
        cout<<"NDV: "<<NDV.estimate()<<endl;
        for(size_t i=0;i<fi.size();i++){
            cout<<"Sketch "<<i<<" : "<<fi[i].estimate()<<endl;
        }
        cout<<"Sketch Size: "<<NDV.sizeofSketch()*(fi.size()+1)<<endl;
    }
    std::vector<uint8_t>& NDVArray(){
        return NDV.Array();
    }
    std::vector<uint8_t>& fiArray(size_t i=0){
        if(i>=fi.size()||i<0){
            cout<<"require error, return f_0"<<endl;
            i=0;
        }
        return fi[i].Array();
    }
};
// Designed for Count Sketch
class Data2SketchPlus{
public:
    HyperLogLog NDV;
    vector<HyperLogLog> fi;
    CountSketch CS;
    uint8_t order;
    Data2SketchPlus(uint8_t b=16,uint8_t level=1, double epsilon=0.01, double gamma=0.1){
        order=level;
        NDV=HyperLogLog(b);
        for(int i=0;i<order;i++){
            HyperLogLog tmp=HyperLogLog(b);
            fi.push_back(tmp);
        }
        CS=CountSketch(epsilon,gamma);
    }
    void ScanString(vector<string> &ss){
        sort(ss.begin(),ss.end());
        int count=1;
    ...
    std::vector<std::vector<int>>& CSArray(){
        return CS.Array();
    }
};
